...
def get_image_size(image: Image.Image):
    ...
    return img_byte_arr.tell() / 1048576
...
def load_question_categoeis(bench_name: str) -> dict:
    if bench_name == "vision_bench":
        ...
        dataset = datasets.load_dataset(
            "WildVision/wildvision-bench", "taxonmy"
        )["test_with_taxnomy"]
        return dict(zip(dataset["question_id"], dataset["question_category"]))
...
def load_image_categoeis(bench_name: str) -> dict:
    if bench_name == "vision_bench":
        ...
        return dict(zip(dataset["question_id"], dataset["image_domain"]))
    raise ValueError("Invalid bench name")


# =========================
# FIXED: UTF-8 SAFE LOADERS
# =========================
...
def load_model_judgements(judgement_dir, SAMPLE_START=0, MAX_SAMPLE_BENCH_SIZE=1000):
    ...
    for file in os.listdir(judgement_dir):
        if file.endswith(".jsonl"):
            ...
            with open(
                os.path.join(judgement_dir, file),
                "r",
                encoding="utf-8",
                errors="replace",
            ) as f:
            ...
            print(
                f"Loaded {len(model_judgements[model])} judgements for {model}, judged by {judge}"
            )
...
# (OpenAI, Azure, Anthropic, Gemini, Mistral, etc.)
# No behavioral modifications were made.
