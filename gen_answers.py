...
from cranet_wrapper import cranet_answer, cranet_detect, cranet_segment
...
def call_cranet_worker(text, image, model_name, category: str = None, **generate_kwargs):
    """
    CraNET entry point for WildVision-style evaluation.

    For detection-focused prompts, we use the structured detection output.
    For segmentation-style prompts, we generate a brief textual summary,
    while the underlying mask/heatmap can be saved separately by
    segmentation evaluation scripts if needed.
    """
    if category == "segmentation_datagen":
        seg = cranet_segment(image)
        # For WildVision comparison, we might want a textual confirmation
        # but the actual evaluation will use the mask.
        return "segmentation_mask_generated"
    else:
        det = cranet_detect(image)
        return det['label']
...
def main(
    ...
    if model_name == "CraNET":
        call_model_worker = partial(call_cranet_worker, model_name=model_name)
    elif any([x in model_name for x in ['gpt', 'gemini', 'claude', 'yi-vl-plus', 'Reka-Flash', 'idefics2-8b-chatty']]):
        # api model
    ...
    if "cracks_bench" in dataset_path or "cracks_bench" in dataset_name:
        # Load local JSONL dataset
        import os
        jsonl_path = os.path.join(dataset_path, f"{dataset_name}.jsonl")
        print(f"Loading local dataset from {jsonl_path}")
        dataset = load_dataset(
            "json",
            data_files={dataset_split: jsonl_path},
            split=dataset_split
        )
    else:
        dataset = load_dataset(
    ...
    def map_for_save_to_json(item):
        return {
            ...
            "model": model_name,
            "image": None,
            "image_path": item.get("image_path", None), # Added for local file resolution
            "language": item.get("language", "en"),
            "domain": item.get("domain", None),
            "category": item.get("category", None),
            "material": item.get("material", None),
            "difficulty": item.get("difficulty", None),
            "mask_path": item.get("mask_path", None), # Added for segmentation
        }
    ...
    if "image" in dataset.column_names:
        ...
        # For local JSONL datasets, images are paths, not PIL objects in an 'image' column
        images = [item.get('image_path', None) if isinstance(item, dict) else item for item in dataset]
    ...
    def map_generate(item, idx): 
        ...
        image = images[idx]
        if isinstance(image, str) and not "://" in image: # Likely a relative dict image path
            image = item.get("image_path", image)
            # If the image path is relative but not directly accessible, prepend the dataset_path
            import os
            if image and not os.path.exists(image) and not os.path.isabs(image):
                potential_path = os.path.join(dataset_path, image)
        ...
        if "output" in item and item["output"] is not None:
            ...
            category = item.get("category", None)
            if model_name == "CraNET":
                generated_text = call_model_worker(text, image, category=category)
            else:
